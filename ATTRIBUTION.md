...
MITRE ATLAS references are used as source-grounded technique and mitigation references. SLEEPER's control mappings, recommended actions, and retest guidance are not official MITRE guidance.
...
OWASP references are used as source-grounded risk categories. SLEEPER's control mappings, recommended actions, and retest guidance are not official OWASP guidance.
...
**ShareAlike note:** OWASP material is CC BY-SA 4.0 while this repository is Apache 2.0. SLEEPER reproduces OWASP identifiers, entry titles, and ID-level mapping relationships with attribution, and does not reproduce OWASP's descriptive or relevance prose. Any future work that adapts substantial OWASP text into this repository needs a licensing decision first.
...
AIUC-1 revises quarterly. Every AIUC-1 citation in this repository records the release date it was read from. SLEEPER does not reproduce the requirement or control set in bulk, and no SLEEPER output is an AIUC-1 conformity assessment or certification.
