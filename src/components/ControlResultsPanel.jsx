...
function OutcomeTag({ C, outcome }) {
  ...
  return (
    <span style={{
      fontSize: C.size.micro, color, fontWeight: 800, letterSpacing: .5,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      <div style={{ padding: '14px 16px', borderRadius: 2, background: C.surface, border: `1px solid ${color}55`, borderLeft: `3px solid ${color}` }}>
        ...
        <div style={{ fontSize: C.size.small, color: C.text1, lineHeight: 1.5, marginBottom: verdict.reason?.text ? 8 : 0 }}>
          {VERDICT_PLAIN_LANGUAGE[verdict.verdict]}
        </div>
        {verdict.reason?.text && (
          <div style={{ fontSize: C.size.small, color: C.text2, lineHeight: 1.5 }}>{humanizeReason(verdict.reason.text)}</div>
        )}
        {verdict.reason?.code && (
        ...
        {Object.entries(CONTROL_LABEL).map(([key, label]) => {
          ...
          return (
            <div key={key} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', background: C.panel, border: `1px solid ${C.border}`, borderRadius: 2, flexWrap: 'wrap' }}>
              <div style={{ flex: '1 1 200px', fontSize: C.size.small, color: C.text1, fontWeight: 600 }}>{label}</div>
              <OutcomeTag C={C} outcome={row.outcome} />
              <div style={{ flex: '2 1 240px', fontSize: C.size.small, color: C.text2, lineHeight: 1.4 }}>{row.text}</div>
            </div>
          );
        })}
      </div>
            ...
            {Object.values(profiles).map(profile => {
              ...
              return (
                <div key={profile.id} style={{
                  ...
                  <div style={{ fontSize: C.size.micro, color: C.text2, fontWeight: 700, marginBottom: 3 }}>{profile.label}</div>
                  <div style={{ fontSize: C.size.micro, color: tone, fontWeight: 800, letterSpacing: .5 }}>
